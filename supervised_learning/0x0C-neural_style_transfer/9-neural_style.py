...
class NST:
    ...
    def generate_image(self, iterations=1000, step=None, lr=0.01,
        ...
        if iterations < 0:
            raise ValueError("iterations must be an positive")
        if not isinstance(step, (int, None)):
        ...
        optimization = tf.train.AdamOptimizer(learning_rate=lr,
                                              beta1=beta1,
                                              beta2=beta2)
        ...
        for i in range(iterations):
            gradient, J_total, J_content, J_style = (self.compute_grads
                                                     (generated_image))
            optimization.apply_gradients([(gradient, generated_image)])
